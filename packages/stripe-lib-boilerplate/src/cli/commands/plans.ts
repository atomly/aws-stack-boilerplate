...
import { StripePriceRecurringIntervals, StripeSupportedCurrencies } from '@atomly/entities-lib-boilerplate';
...
export async function handler(rawArgs: unknown): Promise<void> {
  ...
  for (const plan of config.stripe.plans) {
    ...
    if (planDocument) {
      ...
      // Update the product, price, and Template plan if the data exists in Stripe:
        ...
        .findOneAndUpdate(
          { uuid: planDocument.uuid },
          {
            ...
            price: {
              ...
              currency: price.currency as StripeSupportedCurrencies,
              unitAmount: price.unit_amount!,
              recurring: {
                interval: price.recurring!.interval as StripePriceRecurringIntervals,
